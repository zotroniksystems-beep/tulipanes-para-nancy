(() => {
  ...
  function burst(){const host=$('#magicParticles');host.replaceChildren();for(let i=0;i<24;i++){const p=document.createElement('i');p.className='magic-particle';p.textContent=i%3?'✦':'♥';const angle=Math.PI*2*i/24,distance=70+Math.random()*100;p.style.cssText=`--x:${Math.cos(angle)*distance}px;--y:${Math.sin(angle)*distance}px;--r:${Math.random()*360}deg;--s:${8+Math.random()*11}px;--c:${i%3===0?'#ff9c9c':'#ffe66b'}`;host.append(p)}}
  buildFlowers($('#flowersBack'),34,false);buildFlowers($('#flowersFront'),38,true);buildParticles();
